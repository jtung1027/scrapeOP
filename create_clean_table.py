...
def create_clean_table_two_ways(df):
    ...
    for i in range(1, df["Bookmaker"].size):
        if (
            ...
            | (df["Home_id"][i] != df["Home_id"][i - 1])
            | (df["Away_id"][i] != df["Away_id"][i - 1])
        ):
            L[i] = 1
    ...
    df_final = pd.DataFrame(
        index=range(max(df["MatchId"])), columns=range(7)
    )  #  Home_id, Away_id, Date, Score_home, Score_away, Season, MatchId
    df_final.columns = [
        "MatchId",
        "Season",
        "Home_id",
